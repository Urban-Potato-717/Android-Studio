...
이 README는 발표 자료를 만드는 팀원이 현재 구현된 앱의 기능, 화면 흐름, 파일 구조, 데이터 흐름을 빠르게 이해하기 위한 문서입니다.
...
발표에서는 이 부분을 “수업에서 배운 기본 위젯과 Activity 구조를 바탕으로, 실제 앱처럼 동작하도록 SQLite와 외부 API를 추가했다”는 방향으로 설명하면 됩니다.
...
처음 받는 팀원은 Android Studio로 프로젝트 루트 폴더를 열면 됩니다.

Git 사용이 처음이라면 [GIT_가이드.md](GIT_가이드.md)를 먼저 보면 됩니다.
...
- 서버 없이 SQLite만으로 회원, 책, 리뷰 데이터를 관리한다.
- 리뷰 작성 후 평균 별점과 리뷰 수가 즉시 다시 계산된다.
- 스포일러 리뷰는 사용자가 확인하기 전까지 가려진다.
- 카카오 API로 앱 내부에 없는 책도 검색해서 리뷰 대상으로 추가할 수 있다.
- 외부 이미지 라이브러리 없이 URL 표지를 직접 비동기 로딩한다.
- 최신순/별점순 정렬은 `AlertDialog` 단일 선택 UI로 구현했다.
- 하단 탭 구조로 홈, 내 리뷰, 마이페이지를 분리했다.
